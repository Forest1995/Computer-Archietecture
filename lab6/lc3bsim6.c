...
int MEM_PCMUX, TARGET_PC, TRAP_PC;
int v_mem_ld_cc, v_mem_ld_reg;
/************************* MEM_stage() *************************/
void MEM_stage() {
	int ii, jj = 0;
	int read_word, write_word, dcache_r;
	int WE1, WE0 = 0;
	...
	if (Get_DCACHE_EN(PS.MEM_CS) && PS.MEM_V) {
		if (Get_DATA_SIZE(PS.MEM_CS))
			...
			write_word = ((PS.MEM_ALU_RESULT & 0x00FF) + ((PS.MEM_ALU_RESULT << 8) & 0xFF00));
	...
	if (Get_DATA_SIZE(PS.MEM_CS) == 0) {
		if (PS.MEM_ADDRESS & 0x0001)
			read_word = (read_word >> 8) & 0x00FF;
		else
			read_word = read_word & 0x00FF;
	}
	if (PS.MEM_V) {
...
void AGEX_stage() {
	...
	int ii, jj = 0;
	int LD_MEM, MEM_ADDRESS, ALU_RESULT;
	...
	if (Get_ADDRESSMUX(PS.AGEX_CS)) {
		...
		MEM_ADDRESS = Low16bits(addr1 + addr2);
	}
	else
		MEM_ADDRESS = Low16bits((PS.AGEX_IR & 0x00FF) << 1);
	if (Get_ALU_RESULTMUX(PS.AGEX_CS)) {
		...
		case 3: ALU_RESULT = Low16bits(sr2); break;
		}
	}
	else {
		if (PS.AGEX_IR & 0x0010) {
			if (PS.AGEX_IR & 0x0020)
				ALU_RESULT = Low16bits(signExtend(PS.AGEX_SR1, 16) >> (PS.AGEX_IR & 0x0F));
			else
				ALU_RESULT = Low16bits(PS.AGEX_SR1 >> (PS.AGEX_IR & 0x0F));
		}
		else
			ALU_RESULT = Low16bits(PS.AGEX_SR1 << (PS.AGEX_IR & 0x0F));
	}
...
void DE_stage() {
	...
	int ii, jj = 0;
	int LD_AGEX, DRID, SR1_ID, SR2_ID, SR1_DATA, SR2_DATA;
	CONTROL_STORE_ADDRESS = ((PS.DE_IR & 0xF800) >> 10) + ((PS.DE_IR & 0x0020) >> 5) & 0x003F;
	...
	SR1_ID = ((PS.DE_IR & 0x01C0) >> 6) & 0x0007;
	SR1_DATA = REGS[SR1_ID];
	if (PS.DE_IR & 0x2000) {
		SR2_ID = ((PS.DE_IR & 0x0E00) >> 9) & 0x0007;
		SR2_DATA = REGS[SR2_ID];
	}
	else {
		SR2_ID = PS.DE_IR & 0x0007;
		SR2_DATA = REGS[SR2_ID];
	}
	if (Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]))
		DRID = 7;
	else
		DRID = (((PS.DE_IR & 0x0E00) >> 9) & 0x0007);
	if (PS.DE_V) {
		if (Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (((SR1_ID == PS.AGEX_DRID) && v_agex_ld_reg) || ((SR1_ID == PS.MEM_DRID) && v_mem_ld_reg) || ((SR1_ID == PS.SR_DRID) && v_sr_ld_reg)))
			dep_stall = 1;
		else if (Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (((SR2_ID == PS.AGEX_DRID) && v_agex_ld_reg) || ((SR2_ID == PS.MEM_DRID) && v_mem_ld_reg) || ((SR2_ID == PS.SR_DRID) && v_sr_ld_reg)))
			dep_stall = 1;
		else if (Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc))
			dep_stall = 1;
		else
			dep_stall = 0;
	}
...
void FETCH_stage() {
	...
	int PC_TEMP, read_word;
	int INCREMENTED_PC = PC + 2;
	if (!MEM_PCMUX)
		PC_TEMP = PC + 2;
	else if (MEM_PCMUX == 1)
		PC_TEMP = TARGET_PC;
		...
		PC_TEMP = TRAP_PC;
	icache_access(Low16bits(PC), &read_word, &icache_r);
	if ((!(dep_stall || mem_stall || v_de_br_stall || v_agex_br_stall || v_mem_br_stall) && icache_r) || (v_mem_br_stall && MEM_PCMUX))
		PC = PC_TEMP;
	int LD_DE = !mem_stall & !dep_stall;
	if (LD_DE) {
		NEW_PS.DE_NPC = Low16bits(INCREMENTED_PC);
